...
should_do_rx_squelch (int k)
{
	if (rx[k].squelch.flag)
	{
		...
		if(10.0 * log10 (rx[k].squelch.power + 1e-17) < rx[k].squelch.thresh)
...
should_do_tx_squelch (void)
{
	if (tx.squelch.flag)
	{
		...
		if((-30 + 10.0 * log10 (tx.squelch.power + 1e-17)) < tx.squelch.thresh)
			tx.squelch.set = TRUE;
		else
...
do_rx_pre (int k)
{
	...
	if (rx[k].osc.gen->Frequency != 0.0)
	{
		...
		for (i = 0; i < n; i++)
			CXBdata (rx[k].buf.i, i) = Cmul (CXBdata (rx[k].buf.i, i), OSCCdata (rx[k].osc.gen, i));
	}
...
do_rx_post (int k)
{
  ...
  if(rx[k].squelch.set)
  {
	...
	if (rx[k].grapheq.flag && rx[k].mode != DRM)
  ...
  // position in stereo field
	...
	if (rx[k].output_gain != 1.0)
			for (i = 0; i < n; i++) CXBdata(rx[k].buf.o,i) = Cscl(CXBdata(rx[k].buf.o,i),rx[k].output_gain);
}
