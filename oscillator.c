...
ComplexOSC(OSC p)
{
    ...
    for (i = 0; i < OSCsize(p); i++) z = CXBdata((CXB)OSCbase(p), i)
       ...
       double Frequency, double Phase, REAL SampleRate, char *tag)
{
    ...
    if((OSCtype(p) = TypeOsc) == ComplexTone) OSCbase(p) = (void *)newCXB(size,
                                                                          NULL,
                                                                          "complex buffer for oscillator output");
    else OSCbase(p) = (void *)newRLB(size,
...
delOSC(OSC p)
{
    if(p->OscillatorType == ComplexTone) delCXB((CXB)p->signalpoints);
    else delRLB((RLB)p->signalpoints);
    if(p) safefree((char *)p);
}
