       ...
       BOOLEAN autowordspacing, int weight, REAL ticklen)
{
    ...
    if(dit && dah)
    {
        ...
        if(kl->timeout.midl <= 0) kl->element.psqam = 1;
    } else
        /* Are both paddles released and we had gotten a squeeze in this element? */
        if(!dit && !dah && kl->element.iamb == PADDLES_SQUEEZED) kl->element.iamb = PADDLES_RELEASED;
    ...
    if(kl->timeout.elem <= 0 && kl->element.curr != NO_ELEMENT)
    {
        ...
        {
            if(kl->element.last == DAH) set_element_timeouts = kl->element.curr = DIT;
            else set_element_timeouts = kl->element.curr = DAH;
        } else
        {
            /* No more element */
    ...
    /* Do the dah memory */
    if(kl->element.curr == DIT && !kl->flag.prev.dah && dah && dahmemory) kl->element.invtd = 1;

    /* Do the dit memory */
    if(kl->element.curr == DAH && !kl->flag.prev.dit && dit && ditmemory) kl->element.invtd = 1;
    ...
    if(kl->timeout.dlay > 0 &&
       ...
       !dah &&
       ((kl->element.curr == DIT &&
         !ditmemory) || (kl->element.curr == DAH && !dahmemory))) set_element_timeouts = kl->element.curr = NO_ELEMENT;
