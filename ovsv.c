...
{
    if(parity & 01) return (pflt->zovec + pflt->buflen);
    else return (pflt->zovec);
}

COMPLEX*
FiltOvSv_storept_par(FiltOvSv pflt, int parity)
{
    if(parity & 01) return (pflt->zovec);
    else return (pflt->zovec + pflt->buflen);
}
...
#else
        for (i = 0; i < ncoef; i++) zcvec[fftlen - ncoef + i] = coefs[i];
#endif
