...
#ifdef LHS
    for (i = 0; i < ncoef; i++) zcvec[i] = rx[RL].filt.coef->coef[i];
#else
    for (i = 0; i < ncoef; i++) zcvec[fftlen - ncoef + i] = rx[RL].filt.coef->coef[i];
#endif
    fftwf_execute(ptmp);
...
setMode(int n, char **p)
{
    ...
    } else tx.mode = rx[RL].mode = uni.mode.sdr = mode;
...
setOsc(int n, char **p)
{
    ...
    } else tx.osc.gen->Frequency = rx[RL].osc.gen->Frequency = newfreq;
    return (0);
}
        ...
        else
        {
            ...
            else
            {
                ...
                if(log && !quiet)
                {
                    ...
                    for (i = 0; i < NF(splt); i++) fprintf(log, " %s", F(splt, i));
                    putc('\n', log);
                    fflush(log);
                }
                // discard val
    ...
    if(n == 1)
    {
        ...
        if(samplerate != uni.samplerate)
        {
            ...
            if(reset_for_samplerate((REAL)atof(p[0])) != -1)
            {
                ...
                if(uni.update.flag) replay_updates();
                rtn = 0;
            }
            top.susp = FALSE;
        }
...
setfixedAGC(int n, char **p)
{
    ...
    } else tx.leveler.gen->gain.now = rx[RL].dttspagc.gen->gain.now = gain;
    return (0);
}
...
setGrphRXEQ(int n, char **p)
{
    ...
    else
    {
        ...
        for (i = 0; i < 257; i++) tmpcoef[i] = Cscl(tmpfilt->coef[i], gain[0]);
        delFIR_Bandpass_COMPLEX(tmpfilt);

        tmpfilt = newFIR_Bandpass_COMPLEX(400, 1500, uni.samplerate, 257);
        for (i = 0; i < 257; i++) tmpcoef[i] = Cadd(tmpcoef[i], Cscl(tmpfilt->coef[i], gain[1]));
        delFIR_Bandpass_COMPLEX(tmpfilt);
        ...
        for (i = 0; i < 257; i++) tmpcoef[i] = Cadd(tmpcoef[i], Cscl(tmpfilt->coef[i], gain[2]));
        delFIR_Bandpass_COMPLEX(tmpfilt);

        tmpfilt = newFIR_Bandpass_COMPLEX(-6000, -1500, uni.samplerate, 257);
        for (i = 0; i < 257; i++) tmpcoef[i] = Cadd(tmpcoef[i], Cscl(tmpfilt->coef[i], gain[2]));
        delFIR_Bandpass_COMPLEX(tmpfilt);
        for (i = 0; i < 257; i++) filtcoef[254 + i] = tmpcoef[i];
        ptmp =
...
setGrphTXEQ(int n, char **p)
{
    ...
    else
    {
        ...
        for (i = 0; i < 257; i++) tmpcoef[i] = Cadd(tmpcoef[i], Cscl(tmpfilt->coef[i], gain[2]));
        delFIR_Bandpass_COMPLEX(tmpfilt);
        for (i = 0; i < 257; i++) filtcoef[255 + i] = tmpcoef[i];
        ptmp =
            fftwf_plan_dft_1d(512, (fftwf_complex *)filtcoef,
...
setSpotTone(int n, char **p)
{
    ...
    } else SpotToneOff(rx[RL].spot.gen);
    return (0);
}
...
{
    int lis = atoi(p[0]);
    if(lis < 0 || lis >= uni.multirx.nrx) return (-1);
    else
    {
        uni.multirx.lis = lis;
        return (0);
...
setRXOn(int n, char **p)
{
    if(n < 1)
    {
        if(uni.multirx.act[RL]) return (-1);
        else
        {
    ...
    {
        int k = atoi(p[0]);
        if(k < 0 || k >= uni.multirx.nrx) return (-1);
        else
        {
            if(uni.multirx.act[k]) return (-1);
            else
            {
...
setRXPan(int n, char **p)
{
    ...
    {
        if((pos = (REAL)atof(p[0])) < 0.0 || pos > 1.0) return (-1);
        theta = (REAL)((1.0 - pos) * M_PI / 2.0);
        rx[RL].azim = Cmplx((REAL)cos(theta), (REAL)sin(theta));
        return (0);
    }
...
setAuxMixSt(int n, char **p)
{
    ...
    } else
    {
        ...
        } else uni.mix.rx.flag = uni.mix.tx.flag = flag;
        return (0);
    }
}
...
setAuxMixGain(int n, char **p)
{
    ...
    } else
    {
        ...
        } else uni.mix.rx.gain = uni.mix.tx.gain = gain;
        return (0);
    }
}
    ...
    if(uni.spec.polyphase != setit)
    {
        if(setit)
        {
            ...
            uni.spec.mask = (8 * uni.spec.size) - 1;
            {
                ...
                for (i = 0; i < 8 * uni.spec.size; i++) MaxTap = max(MaxTap, (REAL)fabs(uni.spec.window[i]));
                MaxTap = 1.0f / MaxTap;
...
setSpectrumWindow(int n, char **p)
{
    ...
    if(!uni.spec.polyphase) makewindow(window, uni.spec.size, uni.spec.window);
...
do_update(char *str, FILE *log)
{
    ...
    {
        str++;          // strip !
        if(uni.update.flag) fputs(str, uni.update.fp);
    }
    // echo to logging output?
    if(*str == '-')
    {
...
{
    char buffer[64];
    if(rx[RL].mode == SPEC) setit = SPEC_SEMI_RAW;
    switch(setit)
    {
    case 0:
...
void SetSquelchState(BOOLEAN setit)
{
    char buffer[64];
...
void SetTRX(TRXMODE setit)
{
    ...
    if(setit == RX)
    {
        for (i = 0; i < uni.multirx.nrx; i++) reset_OvSv(rx[i].filt.ovsv);
    } else reset_OvSv(tx.filt.ovsv);
...
void
SetRingBufferOffset(int offset)
{
    char buffer[64];
    sprintf(buffer, "!setRingBufferOffset %d\n", offset);
    sendcommand(buffer);
}
...
void
SetRXListen(int rx_index)
{
    char buffer[64];
    sprintf(buffer, "!setRXListen %d\n", rx_index);
    sendcommand(buffer);
    return;
}

void
SetRXOn(int rx_index)
{
    char buffer[64];
    sprintf(buffer, "!setRXOn %d\n", rx_index);
    sendcommand(buffer);
    return;
}

void
SetRXOff(int rx_index)
{
    char buffer[64];
    sprintf(buffer, "!setRXOff %d\n", rx_index);
    sendcommand(buffer);
    return;
}
...
{
    char buffer[64];
    sprintf(buffer, "!setRXPan %f\n", p);
    sendcommand(buffer);
    return;
}

void
SetRxIqCorrectEnable(int data)
{
    rxCorrEna = data;
    return;
}
