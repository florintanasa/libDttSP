...
DttSPAgc(DTTSPAGC a, int tick)
{
    ...
    if(a->mode == 0)
    {
        for (i = 0; i < CXBsize(a->buff); i++) CXBdata(a->buff, i) = Cscl(CXBdata(a->buff, i), a->gain.fix);
        return;
    }
    for (i = 0; i < CXBsize(a->buff); i++)
    {
        ...
        if(tmp != 0.0) tmp = a->gain.limit / tmp;  // if not zero sample, calculate gain
        else tmp = a->gain.now;  // update. If zero, then use old gain
        if(tmp < hangthresh) a->hangindex = hangtime;
        if(tmp >= a->gain.now)
        {
        ...
        if(tmp != 0.0) tmp = a->gain.limit / tmp;
        else tmp = a->gain.fastnow;
